"""Целевая политика и функция ценности с явными намерениями."""
...
class HIQLwithIntentionsAgent(flax.struct.PyTreeNode):
    """Добавляет явные намерения и две оценки ценности."""
    rng: Any
    network: Any
...
def get_config():
    config = ml_collections.ConfigDict(
        dict(
            ...
            agent_name='iqlintentions',  # Название реализации агента.
            ...
            # Специальные настройки двойной оценки ценности.
            latent_dim = 128,  # Размерность скрытого FB-представления.
            # Ранее использовавшийся вариант: p_same_goals=0.5, # Probability of goal and intention being the same state
        )
    )
    return config